...
class RetrainEnv(gym.Env):
    def __init__(self, data_file_name: str = "data.txt"):
...
class RetrainPPO(PPO):
    def __init__(self, 
        ...
        self.experiment_data = data.to(self.device)
    ...
    def collect_rollouts(
        ...
        while n_steps < n_rollout_steps:
            ...
            with th.no_grad():
                ...
                actions = self.experiment_data[self.num_timesteps, 0]
                values = self.experiment_data[self.num_timesteps, 1]
                log_probs = self.experiment_data[self.num_timesteps, 2]
...
if __name__ == '__main__':
    env = RetrainEnv("data.txt")
    train_data = env.get_train_data()
    model = RetrainPPO("MlpPolicy", env, train_data, verbose=1, tensorboard_log="./ppo/")
